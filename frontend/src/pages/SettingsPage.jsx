    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div><label className="block text-xs font-medium text-neutral-600 mb-1">Angebot gültig (Tage)</label>
            <input type="number" value={offerValidDays} onChange={e => setOfferValidDays(e.target.value)} className="w-full border border-neutral-200 rounded-lg px-3 py-2 text-sm" min={0} max={365} />
            <p className="text-xs text-neutral-400 mt-1">0 = keine Vorbelegung</p></div>
        </div>
...
// ── Tab: Kontenplan → umgezogen ──────────────────────────────────────────────
//
// Der Kontenplan lag hier als Reiter und war damit nur für Admins erreichbar.
// Er ist Arbeitsmittel der Buchhaltung, nicht Konfiguration der Anwendung, und
// steht jetzt unter Buchhaltung → Kontenplan (pages/KontenplanPage.jsx).
//
// Der Reiter bleibt vorerst als Wegweiser stehen: Wer ihn hier gewohnt ist,
// sucht sonst. Ändern darf die Konten weiterhin nur ein Admin — das prüft das
// Backend, nicht die Oberfläche.
function TabKontenplanHinweis() {
  const navigate = useNavigate()
  return (
    <div className="rounded-xl border border-neutral-200 bg-neutral-50 p-5">
      <h3 className="text-sm font-semibold text-neutral-700 mb-1">Der Kontenplan ist umgezogen</h3>
      <p className="text-sm text-neutral-600 mb-4">
        Du findest ihn jetzt im Bereich <strong>Buchhaltung</strong>. Dort steht er allen
        zur Verfügung, die mit den Zahlen arbeiten — geändert werden darf er weiterhin
        nur von einem Administrator.
      </p>
      <button onClick={() => navigate('/buchhaltung/konten')}
        className="inline-flex items-center gap-1.5 px-3 py-2 text-sm bg-primary-600 text-white rounded-lg hover:bg-primary-700">
        Zum Kontenplan
      </button>
    </div>
  )
...
function TabParameter() {
  ...
  return (
    <div>
      ...
      {sub === 'konten'        && <TabKontenplanHinweis />}

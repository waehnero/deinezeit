...
import {
  ArrowLeft, Plus, Trash2, Loader2, GanttChartSquare, Tag as TagIcon, X,
} from 'lucide-react'
...
function OptionListEditor({ items, onChange }) {
  ...
  const add = () => onChange([...items, { value: '', label: '', color: '#6b7280' }])
    ...
    <div className="space-y-2">
      {items.map((it, i) => (
        <div key={i} className="flex items-center gap-2">
          ...
          <input
            value={it.label}
            onChange={(e) => update(i, { label: e.target.value, value: it.value || e.target.value.toLowerCase().replace(/\s+/g, '_') })}

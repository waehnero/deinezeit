 ...
 * Wenn der EntityType Tabs definiert hat, werden diese als Reiter angezeigt.
...
export default function DynamicForm({ fields, tabs = [], values, onChange, disabled = false }) {
  const sorted = [...fields].sort((a, b) => a.sort_order - b.sort_order)
  const [activeTab, setActiveTab] = useState(tabs.length > 0 ? tabs[0] : null)

  // Wenn tabs sich ändern (z.B. nach Reload), aktiven Tab anpassen
  useEffect(() => {
    if (tabs.length > 0 && (!activeTab || !tabs.includes(activeTab))) {
      setActiveTab(tabs[0])
    }
    if (tabs.length === 0) setActiveTab(null)
  }, [tabs])
  ...
  // Felder für aktuellen Tab filtern (oder alle wenn keine Tabs)
  const visibleFields = tabs.length > 0 && activeTab
    ? sorted.filter(f => (f.tab || tabs[0]) === activeTab)
    : sorted
    ...
    <div>
      ...
      <div className="grid grid-cols-12 gap-4">
        ...
        {visibleFields.length === 0 && (
          <div className="col-span-12 py-6 text-center text-gray-400 text-sm">
            Keine Felder in diesem Register.
          </div>
        )}
      </div>
    </div>
...
function InlineCreateModal({ entityType, onClose, onCreated }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-[60] flex items-start justify-center p-4 overflow-y-auto sheet-safe">
      <div className="max-h-full overflow-y-auto bg-surface rounded-2xl shadow-2xl w-full max-w-xl my-8">
